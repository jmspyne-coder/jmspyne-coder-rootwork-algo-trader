...
3. **Per-trade outcomes are not reconciled.** `execute_orb.py` logs the entry to `algo_trade_log` at fill time with `exit_reason='open'` and no realized P&L; the bracket resolves server-side later and is never written back. Realized P&L lives only in `algo_daily_summary` (daily granularity). This is why risk derivation and consecutive-loss counting are day-based, and it is the gap the leak-finder task must close before per-trade analytics are possible.
